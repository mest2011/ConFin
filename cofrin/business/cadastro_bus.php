...
class CadastroUsuario extends Crud
{
    ...
    static function salvarUsuarioGoogle($pessoa)
    {
        try {
            $db = new Db();
            $conn = $db->connect();
            $sql = "INSERT INTO tb_pessoa (nome, email) VALUES 
        ('" . $pessoa->nome . "','" . $pessoa->email . "');
        ";
            if ($conn->query($sql) === TRUE) {
                $last_id = $conn->insert_id;

                $sql = "INSERT INTO tb_usuario (usuario, senha, userIDGoogle, userToken, id_pessoa) VALUES ('" . $pessoa->usuario . "', '" . $pessoa->senha . "', '" . $pessoa->userIDGoogle . "', '" . $pessoa->userToken . "', 
            $last_id);";
