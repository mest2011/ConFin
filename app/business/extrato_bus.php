...
class ExtratoBus extends Crud
{
    ...
    public static function buscar($id_usuario, $dt_inicio = "0", $dt_fim = "0")
    {
        ...
        $sql = "SELECT 'Tranferência' as titulo, 'Entre carteiras' as tipo, concat('Conta origem: ', c.nome_carteira , '<br> Conta destino: ', d.nome_carteira) AS descricao, valor,  convert(tt.data_criacao, date) AS data, DATE_FORMAT(tt.data_criacao, '%d/%m/%Y') as data_ptbr, concat(c.nome_carteira,' => ',d.nome_carteira) AS nome_carteira, '📬' as icone FROM tb_transferencia AS tt
                ...
                UNION
                SELECT titulo, tipo, descricao, valor, tb_ganho.data_do_credito AS data, DATE_FORMAT(data_do_credito, '%d/%m/%Y') as data_ptbr, (select nome_carteira from tb_carteira where id_carteira = tb_ganho.id_carteira limit 1) AS nome_carteira, icone FROM tb_ganho WHERE id_usuario = {$id_usuario} AND status = 1
                UNION
                SELECT titulo, tipo, descricao, (valor * (-1)) AS valor, tb_despesa.data_do_debito AS data, DATE_FORMAT(data_do_debito, '%d/%m/%Y') as data_ptbr, (select nome_carteira from tb_carteira where id_carteira = tb_despesa.id_carteira limit 1) AS nome_carteira, icone FROM tb_despesa WHERE id_usuario = {$id_usuario} AND status = 1 ORDER BY data DESC;";
